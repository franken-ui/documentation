...
<span class="uk-label uk-label-primary">Note</span> Since images and video are
responsive by default in UIkit, it's mandatory to set the aspect ratio when
...
| `.uk-box-shadow`        | Add this class to apply a box shadow.            |
...
<span class="uk-label uk-label-primary">Note</span> To inject an SVG logo as
inline SVG, use the [SVG component](/docs/svg). */}
...
Utility component within the `ui()` plugin.
