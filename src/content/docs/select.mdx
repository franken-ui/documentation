...
<div class="uk-alert uk-alert-danger mt-6">
  ...
  <div class="uk-alert-description">
    This component is still in its early stages of development. As a result, you may encounter bugs or unexpected behavior. Please use with caution and report any issues you encounter.
  </div>
</div>
...
<span class="uk-label uk-label-primary">Note</span> When utilizing the `multiple` attribute, note that it will automatically append `[]` to the name of your input. This is particularly useful when your server expects multiple values, even if the user selects only one option. This ensures that your server-side logic can properly handle the input, regardless of the number of values selected.
...
<Example component="select" index={7} />

## Attributes

The following attributes are available for the custom select component:
...
On your `tailwind.config.js` file, add the Combobox component within the `ui()` plugin. Please note that the `.uk-select` name was already occupied by the native `<select>` tag, so the `.uk-combobox` name was chosen instead to avoid conflicts.
