...
text: Background
title: Background
lead: A collection of utility classes to add different backgrounds to elements.
meta:
...
<span class="uk-label uk-label-primary">Note</span> To adapt your content for
better visibility on each background, add the helper classes like
`text-*-foreground`. Use the [Padding component](/docs/padding) to add some
...
<span class="uk-label uk-label-primary">Note</span> When using these classes,
the background position automatically shifts to the middle and background-repeat
is set to no-repeat.
