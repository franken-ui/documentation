...
lead: Learn how to configure and build a dark mode switcher using Franken UI.
meta:
  description: Learn how to configure and build a dark mode switcher using Franken UI.
javascript: none
---

Offering a dark version for your website is more of an expected feature rather than a nice to have nowadays. Luckily, Tailwind CSS has a dark mode functionality enabled and the components from Franken UI also support it by using the "class" option. If you run the `npx franken-ui init -p` command, everything is already setup for you.
