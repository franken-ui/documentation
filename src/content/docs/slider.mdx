...
<span class="uk-label uk-label-primary">Note</span> In this example, we add the
`.uk-width-3-4` class to each item, which makes the slider look very similar to
a slideshow.
...
<span class="uk-label uk-label-primary">Note</span> This example is set to 70%
...
Slider component within the `ui()` plugin.
