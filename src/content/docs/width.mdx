...
text: Width
title: Width
lead: Define the width of elements for different viewport sizes.
meta:
...
<span class="uk-label uk-label-primary">Note</span> We remove redundancy into
each set of unit classes, so that for instance instead of `.uk-width-3-6` you
should use `.uk-width-1-2`.
...
Width component within the `ui()` plugin.
