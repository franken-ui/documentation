...
<p class="uk-text-lead uk-paragraph">{`Attach modals to keyboard shortcuts, allowing for easy toggling and improved user interaction.`}</p>
...
You can add items by simply adding a plain old `<a>` tag. Franken WC will parse items behind the scenes.
...
To group related elements, simply use the `data-group` attribute, and Franken WC will automatically sort the items and add headers.
...
Note that when you hit the Enter key, Franken WC will trigger `.click()` on the active item. This allows you to use `hx-*` attributes and leverage HTMX.
